...
export default defineNuxtConfig({
  ...
  // Self-hosted Umami analytics (analytics.nook.sh). host/id are provided
  // via NUXT_UMAMI_HOST / NUXT_UMAMI_ID env vars (module convention - see
  // https://umami.nuxt.dev/api/configuration), set only in Cloudflare
  // Workers Builds' production environment. Preview builds and local dev
  // have neither var set, so the module has no host/id to inject and the
  // script tag never renders there - keeps analytics data clean of
  // non-production traffic without needing a manual gate here.
  umami: {
