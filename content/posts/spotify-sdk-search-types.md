...
In practice, TypeScript would happily let you write `result.albums.items` after searching for
`track` only. It compiles fine, then hands you a runtime crash the moment that code actually
runs, because `albums` was never in the response to begin with. It's about the worst kind of
typing bug: it doesn't just fail to help, it actively lies to the compiler on your behalf.
...
If you search with a literal, fixed list of types, say `search(q, ['track'])`, TypeScript can
...
types isn't known at compile time (a `string[]` built at runtime, for example) that the compiler
can't promise anything, and every property should be optional.

The real requirement, then, was to make properties optional except when TypeScript can
statically prove which ones will exist. That's not a straightforward "add a `?` to everything"
fix. It needs the type to behave differently depending on whether the input is a fixed tuple or
an open array, decided entirely at the type level.
...
Twenty minutes later, [wonderflame][3] posted an answer that was one of the best pieces of
TypeScript explanation I've read on that site. It didn't just hand me a type, it built the
reasoning up from first principles, step by step, enough that I could turn it directly into a
pull request. Here's that reasoning, laid out the way it earned it.
...
widen it to `ItemTypes[]`. That means a generic `const` type parameter (TypeScript 5.0+, though a
...
SDK, just by using it for something of my own and tripping over a rough edge. That's basically
how all my open-source contributions happen: I'm not trying to contribute, the library is just
in my way and I fix what's broken for me. This one just happened to also teach me a TypeScript
trick I didn't know existed.
